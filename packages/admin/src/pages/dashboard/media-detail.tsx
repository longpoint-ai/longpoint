...
import { Calendar, Download, ImageIcon, Sparkles, Trash2 } from 'lucide-react';
...
export function MediaDetail() {
  ...
  const getClassifierRunStatusBadgeVariant = (
    status: string
  ): 'default' | 'secondary' | 'destructive' | 'outline' => {
    switch (status) {
      case 'SUCCESS':
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          {/* Analysis Section */}
          {primaryAsset?.classifierRuns &&
            primaryAsset.classifierRuns.length > 0 && (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Insights generated by classifiers
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="space-y-6">
                    {primaryAsset.classifierRuns.map((run) => (
                      <div
                        key={run.id}
                      ...
                      >
                        <div className="flex items-start justify-between gap-4">
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              <Badge
                                variant={getClassifierRunStatusBadgeVariant(
                                  run.status
                                )}
                                className="text-xs"
                              >
                                {enumToTitleCase(run.status)}
                              </Badge>
                            </div>
                            {run.classifier.description && (
                              <p className="text-xs text-muted-foreground">
                                {String(run.classifier.description)}
                              </p>
                            )}
                          </div>
                        </div>
                        ...
                        {run.status === 'SUCCESS' && run.result && (
                          <div className="space-y-3 pt-2 border-t">
                            ...
                            {'tags' in run.result &&
                              Array.isArray(run.result.tags) &&
                              run.result.tags.length > 0 && (
                                <div>
                                  <FieldLabel className="text-xs mb-2">
                                    Tags
                                  </FieldLabel>
                                  <div className="flex flex-wrap gap-2">
                                    {(run.result.tags as string[]).map(
                            ...
                            {'Description' in run.result &&
                              typeof run.result.Description === 'string' && (
                                <div>
                                  <FieldLabel className="text-xs mb-2">
                                    Description
                                  </FieldLabel>
                                  <p className="text-sm text-muted-foreground leading-relaxed">
                                    {run.result.Description as string}
                                  </p>
                                </div>
                              )}

                            {/* Display other result fields that aren't tags or Description */}
                            {Object.entries(run.result)
                              ...
                              .map(([key, value]) => (
                                <div key={key}>
                                  <FieldLabel className="text-xs mb-2 capitalize">
                        ...
                        {run.completedAt && run.status === 'SUCCESS' && (
                          <div className="pt-2 border-t">
                            <p className="text-xs text-muted-foreground">
                              Completed{' '}
                              {new Date(run.completedAt).toLocaleString()}
